    ...
    $.JFZ_Dialog2 = function() {
        ...
        plugin.init = function() {
            ...
            plugin.dialog = jQuery('<div>', {
            	//对话框的样式
            	'id':           'jfz_dialog',
                'class':        'jfz_dialog' + (plugin.settings.custom_class ? ' ' + plugin.settings.custom_class : ' dialog_wrap')
            }).css({
                'position':     (plugin.isIE6 ? 'absolute' : 'fixed'),  //  遮罩层定位,非IE6为fixed定位,因IE6不支持fixed,设置absolute来hack
            ...
            if (plugin.settings.title){
                    ...
                    if(plugin.settings.subtitle == ''){
                        var $html = '<i class="icon f_fl"></i><span class="f_fl">'+plugin.settings.title+'</span><a id="dialog_close" class="f_fr" href="javascript:;"></a>';
                    }else{
                        var $html = '<i class="icon f_fl"></i><span class="comy_name f_fl">'+plugin.settings.subtitle+'</span><span class="f_fl">'+plugin.settings.title+'</span><a id="dialog_close" class="f_fr" href="javascript:;"></a>';
                    }
	                // 创建标题
	                jQuery('<h2>', {
	                    'class':    'dialog_title'	//标题的样式
	                    //'id'   :    'JFZDialog_Title'
	                }).html($html).appendTo(plugin.dialog);
	               ...
	               plugin.dialog.delegate(".JFZDialog_Title_Close", "click", function(e){
	                ...
	                //为XX绑定事件
	                plugin.dialog.delegate("#dialog_close", "click", function(e){
                        plugin.close();
                    });
				}
            ...
            plugin.message = jQuery('<div>', {
                // 根据对话框不同的类型  加载不同的样式
                'class':    plugin.settings.dialog_body_class + (get_type() != '' ? ' JFZDialog_Icon JFZDialog_' + get_type() : '')
            });
            ...
            if (plugin.settings.source && typeof plugin.settings.source == 'object') {
                ...
                for (var type in plugin.settings.source) {
                    switch (type) {
                        ...
                        case 'inline':
                            // 直接添加到canvas中
                            var $t = plugin.settings.source[type];
                            canvas.append($t);
                            break;
                    }
